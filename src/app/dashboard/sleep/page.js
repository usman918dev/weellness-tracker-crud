...
export default function SleepPage() {
  ...
  useEffect(() => {
    async function fetchLogs() {
      ...
      setError(null);
      try {
        const res = await fetch("/api/wellness/logs?type=sleep");
  ...
  // Prepare chart data (sleep duration in hours)
  const chartData = {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-gray-900">Sleep</h1>
      <div className="bg-white rounded-lg shadow p-4">
        <Line data={chartData} />
      </div>
      <LogForm type="sleep" onSubmit={async (log) => {
        try {
      ...
      <div className="mt-8">
        <h2 className="text-lg font-semibold mb-2">Your Sleep Logs</h2>
        {loading ? (
